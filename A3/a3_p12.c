...
int main(){
    ...
    /* Uses strcmp to compar string four (copy of one) 
    and two */
    ...
    /* uses for loop with function strchr to print all positions
    of character c in string two */
    int count = 0;
    for(char *p = two; p != NULL; p++, count++){
        p = strchr(p, c);
        if(p == NULL){
            if (count == 0){
